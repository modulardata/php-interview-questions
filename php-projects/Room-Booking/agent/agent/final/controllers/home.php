...
class Home extends CI_Controller {
    ...
    public function forgot_password() {
        ...
        $getpassword = $this->Hotel_Model->get_forgot_password($loginEmailId);
        $agent_no = $getpassword->agent_no;
        if ($getpassword) {

            $ci = get_instance();
            $ci->load->library('email');
            $ci->email->from('it@travelpd.com', 'Finalbooking');
            $list = $loginEmailId;
            $ci->email->to($list);
...
<div>
<p>Greetings From finalbooking,</p>

</div>
<div>Login Details :
<p>UserName : ' . $loginEmailId . '</p>
</div>
<div>Please Reset you password from here <a href="http://www.tickmango.com/home/load_forgot_password/' . $agent_no . '/' . $data . '">click here</a>
...
<div>Registration & finalbooking Team,
</div>
...
');
    ...
    function restore_password() {
        ...
        $agent_info = $this->Hotel_Model->getAgentdetail($email_id, $agent_no);
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($this->Hotel_Model->update_password($agent_id, $password)) {
                $data['status'] = 1;
            } else {
    ...
    public function privacy() {
        ...
        $data['privacy'] = $this->Home_Model->get_description(1);
        //echo '<pre>';         print_r($data); exit;
    ...
    function agent_register() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $email_check = $this->Hotel_Model->check_email_availability($agent_email);
            ...
            if ($email_check != '' || !empty($email_check)) {
                ...
                if (!$this->upload->do_upload('agency_logo')) {
                    ...
                    if (!$this->image_lib->resize()) {  //Resize image
                        ...
                        $this->load->view('home/agent_register', $data); //If error, redirect to an error page
                    } else {
                        unlink($upload_data["full_path"]);

                        $image_path = WEB_DIR . 'admin/upload_files/b2b/images/' . $agent_email . '/logos/agent_logo.png';

                        if ($this->Hotel_Model->add_agent($agent_email, $agent_password, $agency_name, $currency_type, $title, $first_name, $middle_name, $last_name, $mobile_no, $office_phone_no, $address, $pin_code, $city, $state, $country, $image_path, $payment_type)) {
                            $data['status'] = '1';
                            ...
                            $ci = get_instance();
                            $ci->load->library('email');
                            $ci->email->from('info@travelpd.com');
...
<p>Greetings From finalbooking,</p>
<p>
Many thanks for your Interest and Submitting Online Agent Registration using finalbooking
...
<div>If you want login, Please  <a href="http://www.travelpd.com/agent/login">click here</a>
<p>Please do not hesitate to contact us on info@travelpd.com for all your Urgent Queries / Reservation or Requirements.</p></div>
<div>
<div>Thanking you,
</div>
...
//        $data['agent_available_balance'] = $this->Home_Model->get_agent_available_balance($agent_no);
        ...
        //  $data['top_hotels'] = $this->Home_Model->get_top_hotels();
        //echo '<pre>';        print_r($data); exit;
        $this->load->view('home/agent_home', $data);
        // $this->load->view('home/graph');
    }
    ...
    public function view_profile() {
        ...
        $data['currency_list'] = $this->Hotel_Model->get_currency_list();
...
//        exit;
            ...
            if (!empty($file_name)) {
                ...
                if (!$this->upload->do_upload('agency_logo')) {
                    ...
                    if (!$this->image_lib->resize()) {  //Resize image
                        ...
                        if ($this->Hotel_Model->update_agent($agent_id, $agency_name, $currency_type, $title, $first_name, $middle_name, $last_name, $mobile_no, $office_phone_no, $address, $pin_code, $city, $state, $country, $image_path, $agent_type)) {
                ...
                if ($this->Hotel_Model->update_agent($agent_id, $agency_name, $currency_type, $title, $first_name, $middle_name, $last_name, $mobile_no, $office_phone_no, $address, $pin_code, $city, $state, $country, $agency_logo, $agent_type)) {
                    redirect('home/view_profile', 'refresh');
                } else {
                    $data['errors'] = 'Your Profile Not Updated. Please try after some time...';
                    $this->load->view('b2b/accounts/view_profile', $data);
    ...
    //BUS SOURCE LIST
    public function bus_source_autolist() {
        ...
        if (isset($_GET['term'])) {
            ...
            $bus_list = $this->Home_Model->fetchbussource($search);

            if (!empty($bus_list)) {
                for ($i = 0; $i < count($bus_list); $i++) {
                    $buscity = $bus_list[$i]['city_name'];
                    $return_arr[] = array(
                        'label' => ucfirst($buscity),
                        'value' => ucfirst($buscity)
                    );
                }
            } else {
                $return_arr[] = array(
    ...
    public function bus_desti_list() {
        ...
        if (isset($_GET['term'])) {
            ...
            $bus_list = $this->Home_Model->fetchbusdesti($search);
...
//        exit;
    ...
    public function hotel_autolist() {
        ...
        if (isset($_GET['term'])) {
            ...
            $return_arr = array();
            $search = $_GET["term"];
            $city_list = $this->Home_Model->get_hotel_list($search);
//echo '<pre>';print_r($city_list);
    ...
    // Hotel Cities Auto List
    public function hotel_autolist_dom() {
        ...
        if (isset($_GET['term'])) {
            ...
            $return_arr = array();
            $search = $_GET["term"];
            $city_list = $this->Home_Model->get_hotel_list_dom($search);
//echo '<pre>';print_r($city_list);
            if (!empty($city_list)) {
                for ($i = 0; $i < count($city_list); $i++) {
