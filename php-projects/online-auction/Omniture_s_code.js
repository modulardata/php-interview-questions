...
s.linkDownloadFileTypes="exe,zip,wav,mp3,mov,mpg,avi,wmv,pdf,doc,docx,xls,xlsx,ppt,pptx,3gp,fla,flv,gz,mp4,mpeg,rm,swf,wvx,ra,rar,wma,jpg,wmx,m4v,"
...
s.isPageView=true;var prev_val;var referrerURL="";s.usePlugins=true
function s_doPlugins(s){s.tnt=s.trackTNT();if(s.getQueryParam('s_kwcid'))
s.pageURL=s.manageQueryParam('s_kwcid',1,1);if(!s.campaign){if(location.href.toLowerCase().indexOf('/blogs/')>-1){s.campaign=s.getQueryParam('cmp,n',':').toLowerCase();}else{s.campaign=s.getQueryParam('c,n',':').toLowerCase();}}
if(!s.campaign){s.campaign=s.getQueryParam('cid').toLowerCase();}
s.campaign=s.getValOnce(s.campaign,'s_cmp',0).toLowerCase();window.ReadLinkAnalysisCookies("prop21,prop22,prop23,prop24,products,prop35","SC_LINKS");var productsFromCookie=s.products;if(!window.ProductsRecorded){if(productsFromCookie!=""){s.products=productsFromCookie;hasProductsFromCookie=true;}else if(window.isHomePage()){window.SendHomePageInital();}else{s.products=(window.RetrieveProductsImpression(document.body)).toLowerCase();}
window.ProductsRecorded=true;}
if(s.isPageView){s.events=s.apl(s.events,"event20",",",2);prev_val=s.getPreviousValue(s.pageName,'gpv_p7_','');TrackFormCompleteOnConfirmation();setRegisteredPathing();trackviewThrough();getGroupDLP();trackSelectedTabOnLoad();setPageType();if(s.pageName=="car:jobs:jobdetails"&&TnT_CurrentLayout=="CareersJobPostingLayout"){window.s.eVar5=GetQueryStringBasedOnName('job');}
...
s.isPageView=false;trackFederatedAcnEmployee();}
s.eVar29=s.crossVisitParticipation(s.campaign,'s_ev29','90','5','>');s.clickThruQuality('psgs,psys,psbs,s_kwcid','event25','event26');s.matchPastValues("event39",s.campaign,'cmpaccenture','|','');s.eVar46=s.getQueryParam('KW_ID');s.eVar46=s.getValOnce(s.eVar46,'v46',0);}
...
function trackSelectedTabOnLoad(){window.s.prop17="";if($("span[tabid]").length){window.s.prop17=$("span[tabid]").text().toLowerCase();}}
...
function trackFederatedAcnEmployee(){var cookieUserType=s.c_r('s_uType');var cookieEnterprise=s.c_r('s_prop26');if(cookieEnterprise!==""&&cookieUserType!=="unknown"){s.prop26='acnemployee';}else{s.prop26='';}}
...
function trackviewThrough(){if(!(window.s.getQueryParam('c')||window.s.getQueryParam('n')||window.s.getQueryParam('cmp')||window.s.getQueryParam('cid'))){var cnCookie=getViewThroughCookie();if(cnCookie!=null&&cnCookie!=""){s.eVar24=cnCookie;s.eVar25=cnCookie;s.events=s.apl(s.events,"event18",",",2);if(cnCookie.indexOf('|')>-1){s.eVar25=cnCookie.substring(0,cnCookie.indexOf('|'));}}}
...
+"%10+sed,y=tm.getYear(),vt=tm.getDate()+'/'+(tm.getMonth() + 1)+'/'+(y<1900?y+1900:y)+' '+tm.getHours()+':'+tm.getMinutes()+':'+tm.getSeconds()+' '+tm.getDay()+' '+tm.getTimezoneOffset(),tcf,tfs=s.gtfs(),"
