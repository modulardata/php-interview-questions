...
class Hoteli extends CI_Controller {
    ...
    public function __construct() {
        parent::__construct();
        $this->load->model('Hotelspro_Hotel_Model');
...
//        $this->Api_Key = "eTluVFEvcWFtU0JVTDhlSGFhMmlxc1p6QW00RnVhSFZFQVFybVVQbEM1ZUVVMTB5SUNReUVOOUlheTAwcmhmRw==";
    ...
    public function hotel_search() {
        ...
        if ($this->form_validation->run() == FALSE) {
            $data['status'] = '';
            $this->load->view('b2b/home/agent_home', $data);
        } else {
            ...
            $noofnights = $this->Hotelspro_Hotel_Model->calculatedateDiff($checkin, $checkout);
            ...
            if (!empty($city)) {
                ...
                if (!empty($session_data)) {
                    ...
                    if ($sess_city == $city && $sess_checkin == $checkin && $sess_checkout == $checkout && $sess_adultvalue == $adultvalue && $sess_childvalue == $childvalue && $sess_room_count == $room_count) {
                        ...
                        $this->Hotelspro_Hotel_Model->delete_hotel_temp_result($this->sess_id);
                    }
                } else {
                    $this->session->set_userdata('hotel_search_activate', '');
                    $this->Hotelspro_Hotel_Model->delete_hotel_temp_result($this->sess_id);
                ...
                $data['hotel_facility'] = $this->Hotelspro_Hotel_Model->get_hotel_facility();
                $data['room_facility'] = $this->Hotelspro_Hotel_Model->get_room_facility();
                $this->session->set_userdata('api_name_h', $api_name_h);
                $this->load->view('hoteli/search_progress', $data);
            }
        }
    }
    ...
    function search_progress() {
        ...
        $data['city'] = $city;
        $data['result'] = $this->Hotelspro_Hotel_Model->fetch_hotel_result($this->sess_id);
//echo '<pre>';print_r($this->sess_id);echo'<br>';print_r($data);exit;
    ...
    function hotelspro_search_availablility() {
        ...
        $city_val = $this->Hotelspro_Hotel_Model->get_city_code($city);
...
//        exit;
        foreach ((array) $hotelResponse as $hnum => $hotel) {
            ...
            $b2b_markup = $this->Hotelspro_Hotel_Model->get_b2b_markup($agent_no);
            $admin_markup_val = $b2b_markup->markup;
            $agent_markup_val = $this->Hotelspro_Hotel_Model->get_agent_markup($agent_no);
            ...
            $contry = $this->Hotelspro_Hotel_Model->get_country($hotel_search_datas['city']);

            $city_name = $this->Hotelspro_Hotel_Model->get_city_code_id($hotel_search_datas['city']);

            // ***************************inserting data into permanent table******************************************************* //
            if ($this->Hotelspro_Hotel_Model->check_hotelspro_p_result($hotelCode) == '') {
                $hotellist = $this->Hotelspro_Hotel_Model->fetch_hotespro_hotel_list($hotelCode);
                $hoteldesc = $this->Hotelspro_Hotel_Model->fetch_hotespro_hotel_desc($hotelCode);
                $hotelamenty = $this->Hotelspro_Hotel_Model->fetch_hotespro_hotel_amenty($hotelCode);
                //if($hotellist!='' AND $hoteldesc!='' AND $hotelamenty!=''){
                $hotellist = $this->Hotelspro_Hotel_Model->insert_into_p_hotelspro($hotellist, $hoteldesc, $hotelamenty);
                //}
            }
// ***********************************inserting data into permanent table************************************** //
        ...
        if (!empty($insertresult)) {
            ...
            $this->Hotelspro_Hotel_Model->insert_temp_results($insertresult);
        }
    }
    ...
    public function hotel_autolist() {
        ...
        if (isset($_GET['term'])) {
            ...
            $return_arr = array();
            $search = $_GET["term"];
            $city_list = $this->Hotelspro_Hotel_Model->get_hotel_list($search);
...
//$hotel_country = $city_list[$i]['country_name'];
    ...
    function hotel_detail($id) {
        ...
        $idd = $this->Hotelspro_Hotel_Model->get_cancel_attrib_new($id);
        if ($idd->hotel_code != '') {
            //echo $idd->api_temp_hotel_id;exit;
            $this->Hotelspro_Hotel_Model->get_cancel_attrib_new_nerw($idd->hotel_code, $idd->api_temp_hotel_id);
        }
        $service = $this->Hotelspro_Hotel_Model->get_searchresult($id);
        ...
        $data['hotel_facility'] = $this->Hotelspro_Hotel_Model->get_facility_details_hotel($hotel_code);
        $data['room_facility'] = $this->Hotelspro_Hotel_Model->get_facility_details_room($hotel_code);
        ...
        if ($data['lat'] != '' && $data['long'] != '') {
            $data['nearby_hotel'] = $this->Hotelspro_Hotel_Model->get_nearby_hotels($data['lat'], $data['long'], $data['hotel_name'], $data['dest'], $data['session']);
        } else {
            $data['nearby_hotel'] = '';
        }
        if ($api == 'hotelspro') {
            $hotel_image = $this->Hotelspro_Hotel_Model->get_hotelbed_hotel_pro_image($hotel_code);
            if ($hotel_image != "") {
...
//        exit;
    ...
    function mapping_fun_all() {
        ...
        $data['result'] = $this->Hotelspro_Hotel_Model->fetch_search_result_map_new_select_session();

        $this->load->view('hoteli/map_all', $data);
    }

    function fetch_search_result_map() {
        $query = $this->Hotelspro_Hotel_Model->fetch_search_result_map_new();
        $map_data = array();
        $cnt = 0;
    ...
    function pro_pre_booking($result_id) {
        ...
        if ($room_count == 1) {
            $service = $this->Hotelspro_Hotel_Model->get_searchresult($result_id);
            $api = $service->api;
            $hotel_code = $service->hotel_code;
            $hotel_name = $service->hotel_name;
            $star = $service->star;
            ...
            $rm_info = array();
            $rm_info[] = $this->Hotelspro_Hotel_Model->fetch_gta_temp_result_room_result_id($sec_res, $result_id);
            $data['room_info'] = $rm_info;
        } else {
            $result_id1 = explode("-", $result_id);
            $service = $this->Hotelspro_Hotel_Model->get_searchresult($result_id1[0]);
            $api = $service->api;
            $hotel_code = $service->hotel_code;
            $hotel_name = $service->hotel_name;
            $star = $service->star;
            ...
            for ($r = 0; $r < count($result_id1); $r++) {
                $rm_info[] = $this->Hotelspro_Hotel_Model->fetch_gta_temp_result_room_result_id($sec_res, $result_id1[$r]);
            }
            $data['room_info'] = $rm_info;
        }
        $data['errordesc'] = '';
        ...
        if ($data['errordesc'] != '') {
            ...
            redirect('hoteli/errorpage/' . $error);
        } else {
            ...
            foreach ($availableHotels as $hnum => $hotel) {
                ...
                if ($currencyv1 != 'EUR') {
                    $c_val1 = $this->Hotelspro_Hotel_Model->get_currecy_details($currencyv1);
...
//            exit;
            ...
            $this->load->view('hoteli/pro_pre_booking', $data);
        }
    }
...
//               exit;
                ...
                $this->Hotelspro_Hotel_Model->Update_room_type_in_search_result($result_id, $hotel_proid, $totalRoomRate, $admin_markup, $agent_markup, $total_amount, $room, $boardType, $currencyv1, $c_val);
            } else {
                redirect('hoteli/pre_booking/' . $result_id, 'refresh');
            }
            ...
            $data['hotel_facility'] = $this->Hotelspro_Hotel_Model->get_facility_details($hotel_code);
            $data['hotelCode'] = $hotel_code;
            $data['star'] = $service->star;
            ...
            $data['country_list'] = $this->Hotelspro_Hotel_Model->get_country_list();
...
//              
                ...
                $this->load->view('hoteli/pre_booking', $data);
            }
        } else {
...
//            print_r($data);
//            exit;
            redirect("hoteli/booking_final", 'refresh');
            //$this->load->view('hoteli/payment/payment.php', $data);
            // redirect("hoteli/payment_process/$result_id/$amount", 'refresh');
        }
    }
...
//        print_r($data4); //exit;
        ...
        redirect('hoteli/payment_process', 'refresh');
    }
...
//echo '<pre>';print_r($data); exit;
        ...
        $data['agent_info'] = $this->Hotelspro_Hotel_Model->getAgentInfo($agent_id);
...
//            exit;
            ...
            redirect("hoteli/deposit_agent/$result_id/$parent_transaction_id/$amount/$payment_id/$random_id", 'refresh');
...
//    echo '<br/><br/>';         
                ...
                $this->load->view('hoteli/payment/payment_progress', $data);
            } else {
    ...
    function deposit_agent($result_id, $parent_transaction_id, $amount) {
        ...
        $agent_id = $this->session->userdata('agent_id');
        $agent_no = $this->session->userdata('agent_no');
        $agent_available_balance = $this->Hotelspro_Hotel_Model->get_agent_available_balance($agent_no);
        ...
        if ($available_balance < $amount) {
            ...
            $this->Hotelspro_Hotel_Model->update_transaction_amount($agent_id, $agent_no, $amount, $desc, $parent_transaction_id);

            $withdraw_amount = $amount;
            $closing_balance = $available_balance - $withdraw_amount;

            $this->Hotelspro_Hotel_Model->update_agent_withdraw_amount($agent_no, $withdraw_amount, $closing_balance);

            redirect("hoteli/booking_final_pay/$result_id/$parent_transaction_id/null/null", 'refresh');
        }
    }
...
//         exit;
        redirect("hoteli/booking_final_pay/$result_id_1/$parent_transaction_id_1/$payment_id/$random_id", 'refresh');
    }
    ...
    function errorpage($error) {
        ...
        $data['error'] = $error;
        $this->load->view('hoteli/error_page', $data);
    }
...
//        $h_child = $ses_hotel_search_data['childvalue'];
        if ($room_count == 1) {
            $service = $this->Hotelspro_Hotel_Model->get_searchresult($result_id);  //From api_temp and api_parmenent
...
//            $h_lon = $service->longitude;
            $trans = $this->Hotelspro_Hotel_Model->transation_detail($parent_transaction_id);
            //echo '<pre>'; 
            //$amount = $trans->total_amount;
            //print_r($amount);
            //exit;// From Transaction Table
            $trans_id = $trans->customer_contact_details_id;
            $contact_info = $this->Hotelspro_Hotel_Model->contact_info_detail_update($trans_id); //From customer_contact_details table
            $con_id = $contact_info->customer_info_details_id;
            $pass_info = $this->Hotelspro_Hotel_Model->pass_info_detail($con_id); //From customer_info_details Table
            $con_id_org = $contact_info->customer_contact_details_id;
        } else {
            //echo 'ekdsfkjsd';
            $result_id1 = explode("-", $result_id);
            $service = $this->Hotelspro_Hotel_Model->get_searchresult($result_id1[0]); //From api_temp and api_parmenent
            $h_hotel_id = $service->hotel_code;
            $h_hotel_name = $service->hotel_name;
            $h_star = $service->star;
            $h_description = $service->description;
...
//            $guestchild = $ses_hotel_search_data['childvalue'];
            ...
            $trans = $this->Hotelspro_Hotel_Model->transation_detail($parent_transaction_id); // From Transaction Table
            $trans_id = $trans->customer_contact_details_id;
            $contact_info = $this->Hotelspro_Hotel_Model->contact_info_detail_update($trans_id); //From customer_contact_details table
            $con_id = $contact_info->customer_info_details_id;
            $pass_info = $this->Hotelspro_Hotel_Model->pass_info_detail($con_id); //From customer_info_details Table
        ...
        $search = $this->Hotelspro_Hotel_Model->get_cancel_attrib_new($result_id);
...
//            $cout = date("Y-m-d", strtotime($ses_hotel_search_data['checkout']));
            ...
            if ($agent_id) {
                ...
                $val_last = $this->Hotelspro_Hotel_Model->inser_customer_book_hotelpro($h_hotel_id, $agent_id, $agent_no, $h_hotel_name, $h_star, $h_description, $h_address, $h_phone, $h_fax, $h_room_type, $h_cancel_policy, $cin, $cout, $date, $roomcountss, $user_id, $nights, $trans_id, $adults, $child, $con_id_org, $dateFromValc, $h_city, $api);
                //$this->Hotelspro_Hotel_Model->inser_customer_book_hotelpro_trans_hotel($trans_id,$ConfirmationNumbervalue,$user_id,$val_last);
                $this->Hotelspro_Hotel_Model->inser_customer_book_hotelpro_trans_hotel($con_id_org, $ConfirmationNumbervalue, $user_id, $val_last, $BookingStatusvalue, $payment_id, $random_id);
                $this->voucher_email($val_last);
//            echo'ticket Generated';
//            exit;
            }
            redirect('hoteli/voucher/' . $val_last, 'refresh');
        }
    }

    function voucher_email($id) {
        $result_view = $this->Hotelspro_Hotel_Model->book_detail_view_voucher1($id);
        $con_id = $result_view->customer_contact_details_id;
        $trans = $this->Hotelspro_Hotel_Model->transation_detail_contact($con_id);
        $contact_info = $this->Hotelspro_Hotel_Model->contact_info_detail_update($con_id);
        $pass_info = $this->Hotelspro_Hotel_Model->pass_info_detail($con_id);
...
//echo '0123654789';
    ...
    function voucher($val_last) {
        ...
        $agent_id = $this->session->userdata('agent_id');
        $data['result_view'] = $this->Hotelspro_Hotel_Model->book_detail_view_voucher1($val_last);
        $con_id = $data['result_view']->customer_contact_details_id;
        $data['contact_info'] = $this->Hotelspro_Hotel_Model->contact_info_detail_update($con_id);
        $data['trans'] = $this->Hotelspro_Hotel_Model->transation_detail_contact($con_id);
        $con_id_pass = $data['contact_info']->customer_info_details_id;
        $data['pass_info'] = $this->Hotelspro_Hotel_Model->pass_info_detail($con_id_pass);
        $hotel_id = $data['result_view']->hotel_code;
        $data['hotel_image'] = $this->Hotelspro_Hotel_Model->get_hotelimage($hotel_id);
        $data['agent_info'] = $this->Hotelspro_Hotel_Model->getAgentInfo($agent_id);
        $data['hotel_decs'] = '';
//        echo "<pre/>";
//        print_r($data);
//        exit;
        $this->load->view('hoteli/ticket', $data);
//       
    }
    ...
    public function conformed_booking() {
        ...
        if ($this->session->userdata('agent_logged_in')) {
            ...
            $data['agent_info'] = $agent_info = $this->Hotelspro_Hotel_Model->getAgentInfo($agent_id);
            $agent_no = $agent_info->agent_no;
            $data['agent_available_balance'] = $this->Hotelspro_Hotel_Model->get_agent_available_balance($agent_no);


            $data['hotel_booking_summary'] = $this->Hotelspro_Hotel_Model->get_hotel_conformed_summary($agent_id);
            $data['Status'] = 'Confirmed History';
            //echo '<pre>';        print_r($data); exit;


            $this->load->view('hoteli/booking_history', $data);
        } else {
            redirect('home/index', 'refresh');
        }
    }
    ...
    public function cancelled_booking() {
        ...
        if ($this->session->userdata('agent_logged_in')) {
            ...
            $data['hotel_booking_summary'] = $this->Hotelspro_Hotel_Model->get_hotel_cancelled_summary($agent_id);
    ...
    public function rejected_booking() {
        ...
        if ($this->session->userdata('agent_logged_in')) {
            ...
            $data['hotel_booking_summary'] = $this->Hotelspro_Hotel_Model->get_hotel_rejected_summary($agent_id);
    ...
    public function booking() {
        ...
        if ($this->session->userdata('agent_logged_in')) {
            ...
            $hotel_booking_summary = $this->Hotelspro_Hotel_Model->get_hotel_booking_summary($agent_id);
            $data['hotel_booking_summary'] = $hotel_booking_summary;
            $data['Status'] = 'Booking History';
            //$data['agent_acc_summary'] = $this->Hotelspro_Hotel_Model->getAgentAccountInfo($agent_id);
            $data['agent_balance_summary'] = $this->Hotelspro_Hotel_Model->get_agent_account_balance($agent_id);
    ...
    public function deposit_history() {
        ...
        $agent_no = $agent_info->agent_no;
        $data['agent_available_balance'] = $this->Hotelspro_Hotel_Model->get_agent_available_balance($agent_no);
        $data['agent_deposit_summary'] = $this->Hotelspro_Hotel_Model->get_agent_deposit_summary($agent_id);
//        echo '<pre>';
//        print_r($data);
//        exit;
        $data['agent_balance_summary'] = $this->Hotelspro_Hotel_Model->get_agent_account_balance($agent_id);
    ...
    function add_deposit_request() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $agent_no = $data['agent_info']->agent_no;
            $available = $this->Hotelspro_Hotel_Model->get_agent_available_balance($agent_id);
...
//            exit;
            ...
            if ($this->Hotelspro_Hotel_Model->add_deposit_request($agent_id, $agent_no, $amount, $value_date, $transaction_mode, $bank, $branch, $city, $transaction_id, $remarks, $available_balance)) {
                $data['status'] = 1;
            } else {
                $data['errors'] = 'Transaction is not done. Please try after some time...';
            }

            redirect('hoteli/deposit_history', 'refresh');
        }
    }
    ...
    function add_markup() {
        ...
        $data['agent_markup_manager'] = $this->Hotelspro_Hotel_Model->get_agent_markup_manager($agent_no);
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($this->Hotelspro_Hotel_Model->add_markup($agent_no, $service_type, $markup)) {
                $data['status'] = 1;
            } else {
                $data['errors'] = 'Markup is not added. Please try after some time...';
            }

            redirect('hoteli/markup_manager', 'refresh');
        }
    }

    public function markup_manager() {
        if (!$this->session->userdata('agent_logged_in'))
            redirect('hoteli/index', 'refresh');
    ...
    function markup_status($markup_id, $status) {
        $this->Hotelspro_Hotel_Model->update_markup_status($markup_id, $status);
        redirect('hoteli/markup_manager', 'refresh');
    }
    ...
    function cancel_booking($id) {
        ...
        $data['result_view'] = $this->Hotelspro_Hotel_Model->view_trans_detail_id($id);
        //echo '<pre>';        print_r($data); exit;
    ...
    function cancel_booking_final__($id) {
        $result_view = $this->Hotelspro_Hotel_Model->view_trans_detail_id_new2($id);
        $h_id = $result_view->hotel_booking_id;
        $view = $this->Hotelspro_Hotel_Model->book_detail_view_voucher1($h_id);
    ...
    function cancel_booking_final($id) {
        $result_view = $this->Hotelspro_Hotel_Model->view_trans_detail_id($id);
        $h_id = $result_view->hotel_booking_id;
        $view = $this->Hotelspro_Hotel_Model->book_detail_view_voucher1($h_id);
